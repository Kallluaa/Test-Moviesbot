...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, SPRT_CHNL, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, HOW_TO_VERIFY, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER, STREAM_BIN, STREAM_URL, STICKER, QUALITY
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, InputMediaPhoto
from pyrogram import Client, filters, enums
from pyrogram.errors import FloodWait, UserIsBlocked, MessageNotModified, PeerIdInvalid
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all, get_name, get_hash, get_cap
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton("ᴏ̨ᴜᴀʟɪᴛʏ", callback_data=f"quality#{key}"),
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton("⭕ 𝗹𝗮𝗻𝗴𝘂𝗮𝗴𝗲𝘀 ⭕", callback_data=f"languages#{key}")
            ]
        )
    try:
        if settings['max_btn']:
...
#--------------------------------------------------Quality Feature added By IT'S LUFFY ---------------------------------------------#
...
async def quality_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 𝖲𝖾𝗅𝖾𝖼𝗍 Quality 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            ...
            g = await query.message.reply_text("🔐")
            ...
            await log_msg.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Fast Download 🚀", url=stream_link),
                                                    InlineKeyboardButton('🖥️ Watch online 🖥️', url=page_link)]]))
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Jᴏɪɴ ᴏᴜʀ Bᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟ ᴛʜᴇɴ ᴜsᴇ ᴍᴇ! 😒", show_alert=True)
            return
        ...
        buttons = [[
            InlineKeyboardButton('✉️ ᴀɴʏ ǫᴜᴇʀʏ ✉️', url='https://t.me/god_luffy_ati')
        ], [
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("🫵 sᴜʙsᴄʀɪʙᴇ​ 🫵", url='https://youtube.com/@elitecraft_studios')
                ],[
                    InlineKeyboardButton("🍀 Mᴏᴠɪᴇ Sᴇᴀʀᴄʜ Gʀᴏᴜᴘ 🍀", url= GRP_LNK)
                ],[
                    InlineKeyboardButton("🍂 Mᴏᴠɪᴇ Cʜᴀɴɴᴇʟ 🍂", url=CHNL_LNK)
                ],[
                    InlineKeyboardButton("🍁 Bᴏᴛꜱ Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟ 🍁", url='https://t.me/elitecraft_studios')
                ],[
                    ...
                    InlineKeyboardButton('❗❗ Dɪsᴄʟᴀɪᴍᴇʀ ❗❗', callback_data='dicl_btn2')
                ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
    ...
    elif query.data == "dicl_btn2":
        buttons = [[
            InlineKeyboardButton('⇍ ʙᴀᴄᴋ ⇏', callback_data='chnl_info')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await client.edit_message_media(
            query.message.chat.id, 
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/elitecraft_support")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            dlt = await message.reply_sticker(random.choice(STICKER), reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
